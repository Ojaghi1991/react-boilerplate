...
export function promise(type = '', action?: string): Array<string> {
  ...
  if (action) {
    ...
    const create = request.indexOf('C') !== -1 ? [
      `CREATE_${str}`,
    ...
    const load = request.indexOf('L') !== -1
      ? [
        `LOAD_${str}`,
    ...
    const update = request.indexOf('U') !== -1
      ? [
        `UPDATE_${str}`,
    ...
    const loadAll = request.indexOf('A') !== -1
      ? [
        `LOAD_ALL_${str}`,
